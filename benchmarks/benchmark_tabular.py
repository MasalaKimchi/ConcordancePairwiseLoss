#!/usr/bin/env python3
"""
Focused Tabular Benchmark for Core Loss Comparison

This benchmark focuses on comparing 5 core loss functions across all tabular datasets:
1. NLL (Negative Log-Likelihood)
2. CPL (Concordance Pairwise Loss)
3. CPL (ipcw) (CPL with IPCW weighting)
4. CPL+NLL (w. GradNorm) (Combined using gradient norm balancing)
5. CPL+NLL (wo. GradNorm) (Combined using fixed scale factors)

Usage:
    conda activate concordance-pairwise-loss
    python benchmarks/benchmark_tabular.py --dataset gbsg2 --runs 5 --epochs 50
    python benchmarks/benchmark_tabular.py --dataset all --runs 3 --epochs 30  # Run on all datasets
"""
...
class TabularBenchmarkTrainer:
    ...
    def train_model(
        ...
        # Initialize combination strategy for CPL(+IPCW)+NLL variants
        loss_combiner = None
        use_gradnorm = False
        if loss_type in ['cpl_nll_gradnorm', 'cpl_nll_fixed', 'cpl_ipcw_nll_gradnorm', 'cpl_ipcw_nll_fixed']:
            loss_combiner = NormalizedLossCombination(total_epochs=self.epochs)
            use_gradnorm = (loss_type in ['cpl_nll_gradnorm', 'cpl_ipcw_nll_gradnorm'])
    ...
    def _compute_loss(
        ...
        elif loss_type in ["cpl_nll_gradnorm", "cpl_nll_fixed"]:
            # Compute individual losses
            nll_loss = neg_partial_log_likelihood(log_hz_2d, event, time, reduction="mean")
            self.cpl_loss.temperature = temperature
            cpl_loss = self.cpl_loss(log_hz_1d, time, event)
            
            if use_gradnorm and loss_combiner:
                # Use GradNorm weighting
                shared_params = list(model.backbone.parameters())
                nll_w, cpl_w = loss_combiner.get_weights_gradnorm(
                    epoch, nll_loss, cpl_loss, shared_params
                )
            elif loss_combiner:
                # Use fixed scale factor weighting
                nll_w, cpl_w = loss_combiner.get_weights_scale_balanced(
                    epoch, nll_loss.item(), cpl_loss.item()
                )
            else:
                # Equal weighting fallback
                nll_w, cpl_w = 0.5, 0.5
            
            loss = nll_w * nll_loss + cpl_w * cpl_loss
        
        elif loss_type in ["cpl_ipcw_nll_gradnorm", "cpl_ipcw_nll_fixed"]:
            # Compute individual losses using IPCW variant for CPL
            nll_loss = neg_partial_log_likelihood(log_hz_2d, event, time, reduction="mean")
            self.cpl_ipcw_loss.temperature = temperature
            cpl_loss = self.cpl_ipcw_loss(log_hz_1d, time, event)
...
class TabularBenchmarkRunner:
    ...
    def run_comparison(self) -> Dict[str, Dict]:
        ...
        print("Core Loss Functions: NLL, CPL, CPL(ipcw), CPL+NLL(GradNorm), CPL+NLL(Fixed)")
        print("=" * 80)
    ...
    def _plot_training_evolution(self, ax, results):
        ...
        method_colors = {'nll': 'blue', 'cpl': 'orange', 'cpl_ipcw': 'red',
                        'cpl_nll_gradnorm': 'green', 'cpl_nll_fixed': 'purple', 
                        'cpl_ipcw_nll_gradnorm': 'brown', 'cpl_ipcw_nll_fixed': 'pink'}
