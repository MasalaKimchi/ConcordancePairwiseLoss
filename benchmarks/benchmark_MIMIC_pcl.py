#!/usr/bin/env python3
"""
MIMIC-IV Chest X-ray Survival Analysis Benchmark with ProtoCoxLoss (PCL)

This benchmark is based on benchmark_MIMIC_v2.py but adds support for:
1. ProtoCoxLoss (PCL) - Geometric contrastive formulation of Cox partial likelihood
2. ProtoCoxLoss with MoCo (PCL+MoCo) - ProtoCoxLoss with momentum contrast queue
   NOTE: PCL+MoCo integration requires custom training loop (currently NotImplementedError)

This benchmark combines training and comprehensive evaluation in a single run:
1. Trains models with different loss functions (NLL, CPL, CCL, PCL, PCL+MoCo)
2. Evaluates with comprehensive metrics during training
...
Usage:
    ...
    python benchmarks/benchmark_MIMIC_pcl.py --epochs 50 --batch-size 64
    
    # Test only ProtoCoxLoss variants
    python benchmarks/benchmark_MIMIC_pcl.py --pcl-only --epochs 25 --batch-size 64
    
    # Quick test with subset
    python benchmarks/benchmark_MIMIC_pcl.py --epochs 5 --batch-size 32 --data-fraction 0.01
"""
...
from mimic.util import MIMICBenchmarkRunner, MIMICBenchmarkTrainer
...
class ComprehensiveMIMICEvaluator:
    ...
    def __init__(self, device: torch.device, dataset_config: DatasetConfig, loss_fn=None):
        """
        ...
        Args:
            ...
            loss_fn: Optional ProtoCoxLoss instance (for prototype access during evaluation)
        """
        self.device = device
        self.dataset_config = dataset_config
        self.loss_fn = loss_fn  # ProtoCoxLoss instance with prototype
    
    def evaluate_model(self, model: torch.nn.Module, dataloader_test, loss_type: str = None) -> Dict[str, float]:
        """
        Comprehensive evaluation matching evaluate_saved_models.py.
        
        For ProtoCoxLoss models, we need to extract features and compute risk scores
        using the prototype.
        """
        model.eval()
        all_log_hz = []
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader_test):
                ...
                # Get predictions
                # For ProtoCoxLoss models, we need to compute risk scores using the prototype
                if loss_type in ["pcl", "pcl_moco"]:
                    # Extract features
                    if hasattr(model, 'base_model'):
                        # FeatureStoringModel wrapper
                        features = model.base_model.backbone.features(x)
                        features = torch.nn.functional.adaptive_avg_pool2d(features, (1, 1))
                        features = torch.flatten(features, 1)
                    elif hasattr(model, 'backbone_q'):
                        # MoCo wrapper - use query encoder
                        features = model.backbone_q(x)
                    else:
                    ...
                    # Get prototype from loss function
                    # Use the prototype from loss_fn if available (passed to evaluator)
                    if self.loss_fn is not None and hasattr(self.loss_fn, 'risk_prototype'):
                        # Use the trained prototype to compute risk scores
                        proto = F.normalize(self.loss_fn.risk_prototype, p=2, dim=0)
                        log_hz = torch.matmul(features, proto) / self.loss_fn.temperature
                    elif hasattr(model, 'base_model') and hasattr(model.base_model, 'risk_head'):
                        # Fallback: use risk_head (not ideal but works)
                        log_hz = model.base_model.risk_head(features)
                    else:
                        # Last resort: use dummy prototype (evaluation won't be accurate)
                        dummy_proto = torch.ones(features.shape[1], device=features.device)
                        dummy_proto = F.normalize(dummy_proto, p=2, dim=0)
                        log_hz = torch.matmul(features, dummy_proto) / 0.07
                        print("Warning: Using dummy prototype for evaluation. Pass loss_fn to evaluator for accurate evaluation.")
                else:
...
class ProtoCoxLossTrainer(MIMICBenchmarkTrainer):
    """Extended trainer that supports ProtoCoxLoss and ProtoCoxLoss with MoCo."""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        # Initialize ProtoCoxLoss
        try:
            from proto_cox_loss import ProtoCoxLoss, ProtoCoxLossTrainer as PCLTrainer
        except ImportError:
            raise ImportError("proto_cox_loss is required. Install the package first.")
        
        # Feature dimension for EfficientNet-B0
        self.feat_dim = 1280
        
        # Initialize custom trainer for ProtoCoxLoss
        self.pcl_trainer = PCLTrainer(
            device=self.device,
            epochs=self.epochs,
        ...
        # Store ProtoCoxLoss instance (will be created per model)
        self.pcl_loss = None
        
        # Store MoCo queue size
        self.moco_queue_size = 1024
    
    def create_model(self, loss_type: str = None) -> torch.nn.Module:
        """Create model, optionally wrapped with MoCo for ProtoCoxLoss."""
        base_model = super().create_model()
        
        # For ProtoCoxLoss, we need to modify the model to also store features
        if loss_type in ["pcl", "pcl_moco"]:
            # Wrap the model to store features
            class FeatureStoringModel(nn.Module):
                ...
                def forward(self, x):
                    # Extract features
                    features = self.base_model.backbone.features(x)
                    features = torch.nn.functional.adaptive_avg_pool2d(features, (1, 1))
                    features = torch.flatten(features, 1)
                    self.last_features = features
                    # Return risk score for compatibility
                    risk = self.base_model.risk_head(features)
                    return risk
                
                def get_features(self):
                    return self.last_features
            
            wrapped_model = FeatureStoringModel(base_model)
            wrapped_model = wrapped_model.to(self.device)
            
            # For MoCo version, wrap with MoCo
            if loss_type == "pcl_moco":
                try:
                    from proto_cox_loss.moco_wrapper import MoCoCoxWrapper
                except ImportError:
                    raise ImportError("proto_cox_loss.moco_wrapper is required for MoCo support.")
                
                # Create a feature extractor from the wrapped model
                class FeatureExtractor(nn.Module):
                    def __init__(self, wrapped_model):
                        super().__init__()
                        self.backbone = wrapped_model.base_model.backbone
                ...
                feature_extractor = FeatureExtractor(wrapped_model)
                feature_extractor = feature_extractor.to(self.device)
                
                # Wrap with MoCo
                self.pcl_moco_wrapper = MoCoCoxWrapper(
    ...
    def _compute_loss(
        ...
        """Compute loss, including ProtoCoxLoss variants."""
        
        # Handle ProtoCoxLoss variants
        if loss_type == "pcl":
            try:
                from proto_cox_loss.loss import ProtoCoxLoss
                from torchsurv.stats.ipcw import get_ipcw
            except ImportError:
                raise ImportError("proto_cox_loss and torchsurv are required for ProtoCoxLoss.")
            
            # Initialize ProtoCoxLoss if needed
            if self.pcl_loss is None:
                self.pcl_loss = ProtoCoxLoss(
            ...
            # Get features from model (stored during forward pass)
            if hasattr(model, 'get_features'):
                features = model.get_features()
            elif hasattr(model, 'last_features'):
                features = model.last_features
            else:
                # Fallback: use log_hz as proxy (not ideal but works)
                features = log_hz.unsqueeze(-1) if log_hz.dim() == 1 else log_hz
                print("Warning: Could not extract features, using log_hz as proxy")
            
            # Get IPCW weights if needed
            ipcw_weights = None
            if self.pcl_loss.use_ipcw:
                event_cpu = event.cpu()
                time_cpu = time.cpu()
                ipcw_weights = get_ipcw(event_cpu, time_cpu).to(self.device)
            
            loss = self.pcl_loss(features, time, event, ipcw_weights)
            return loss
        
        elif loss_type == "pcl_moco":
            # MoCo wrapper handles loss computation internally
            # This is handled by the custom trainer, not here
            raise NotImplementedError(
                "pcl_moco should use custom trainer, not _compute_loss"
            )
        
        # Fall back to parent class for other loss types
    ...
    def train_model(
        ...
        """Train model, using custom trainer for ProtoCoxLoss variants."""
        
        # Use custom trainer for ProtoCoxLoss variants
        if loss_type in ["pcl", "pcl_moco"]:
            # Create ProtoCoxLoss instance
            from proto_cox_loss import ProtoCoxLoss
            ...
            # Use custom trainer
            return self.pcl_trainer.train_model(
                model=model,
                loss_fn=self.pcl_loss,
        ...
        # Use standard trainer for other loss types
        return super().train_model(model, dataloader_train, dataloader_val, loss_type, temperature)


class PreprocessedMIMICBenchmarkRunnerPCL(MIMICBenchmarkRunner):
    """Enhanced benchmark runner with ProtoCoxLoss support."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize preprocessed MIMIC benchmark runner with ProtoCoxLoss support.
        ...
        """
        # Override trainer with ProtoCoxLoss trainer
        kwargs['trainer_class'] = ProtoCoxLossTrainer
        super().__init__(data_dir=data_dir, csv_path=csv_path, **kwargs)
        self.data_fraction = data_fraction
        
        # Replace trainer with ProtoCoxLoss trainer
        self.trainer = ProtoCoxLossTrainer(
            device=self.device,
            epochs=self.epochs,
            learning_rate=self.learning_rate,
            weight_decay=self.weight_decay,
        ...
        self.comprehensive_evaluator = ComprehensiveMIMICEvaluator(
            self.trainer.device, 
            self.dataset_config,
            loss_fn=None  # Will be set per loss type
        )
    ...
    def run_comparison(self, args=None, loss_types=None) -> dict:
        ...
        if loss_types is None:
            loss_types = ['nll', 'cpl_ipcw', 'cpl_ipcw_batch', 'ccl_ipcw', 'ccl_ipcw_batch', 'pcl', 'pcl_moco']
        
        print("=" * 80)
        print("MIMIC-IV CHEST X-RAY SURVIVAL ANALYSIS BENCHMARK WITH PROTOCOXLOSS")
        ...
        for loss_idx, loss_type in loss_pbar:
            ...
            for run_idx in run_pbar:
                ...
                model = self.trainer.create_model(loss_type=loss_type)
                training_results = self.trainer.train_model(
                    model, dataloader_train, dataloader_val, loss_type, temperature=1.0
                )
                
                # Update evaluator with loss_fn for ProtoCoxLoss models
                if loss_type in ["pcl", "pcl_moco"]:
                    if hasattr(self.trainer, 'pcl_loss') and self.trainer.pcl_loss is not None:
                        self.comprehensive_evaluator.loss_fn = self.trainer.pcl_loss
                
                # Comprehensive evaluation on validation set
                print(f"  Comprehensive evaluation on validation set...")
                val_eval = self.comprehensive_evaluator.evaluate_model(model, dataloader_val, loss_type=loss_type)
                val_comprehensive_results.append(val_eval)
                
                # Comprehensive evaluation on test set
                print(f"  Comprehensive evaluation on test set...")
                test_eval = self.comprehensive_evaluator.evaluate_model(model, dataloader_test, loss_type=loss_type)
    ...
    def _save_best_model(
        ...
        # For ProtoCoxLoss models, save the prototype
        if loss_type in ["pcl", "pcl_moco"]:
            if hasattr(self.trainer, 'pcl_loss') and self.trainer.pcl_loss is not None:
                from proto_cox_loss import save_protocox_model
                save_protocox_model(
                    model=model,
                    loss_fn=self.trainer.pcl_loss,
                    save_path=model_path,
                    metadata=model_save_dict
                )
            else:
                # Fallback: save normally but warn
                print(f"Warning: ProtoCoxLoss instance not found, saving model without prototype")
    ...
    def _save_comprehensive_results(self, results: Dict, args):
        ...
        method_names = {
            ...
            'pcl': 'PCL (ProtoCoxLoss)',
            'pcl_moco': 'PCL+MoCo (ProtoCoxLoss with Momentum)'
        }
        ...
        summary_csv_path = os.path.join(eval_dir, f"benchmark_pcl_summary_{timestamp}.csv")
        ...
        json_path = os.path.join(eval_dir, f"benchmark_pcl_detailed_{timestamp}.json")
        ...
        detailed_csv_path = os.path.join(eval_dir, f"benchmark_pcl_per_run_{timestamp}.csv")
    ...
    def _print_comprehensive_summary(self, results: Dict):
        ...
        print("-" * 120)
        print(f"{'Method':<25} {'Uno C':<18} {'Harrell C':<18} {'Incident AUC':<18} {'Brier Score':<18}")
        print("-" * 120)
        ...
        for loss_type, result in results.items():
            ...
            print(f"{method_name:<25} {uno_mean:.4f}±{uno_std:.4f}    {harrell_mean:.4f}±{harrell_std:.4f}    {incident_mean:.4f}±{incident_std:.4f}    {brier_mean:.4f}±{brier_std:.4f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="MIMIC-IV Benchmark with ProtoCoxLoss: Training + Comprehensive Evaluation"
    )
    ...
    parser.add_argument('--output-dir', type=str, default='results-pcl', help='Output directory')
    ...
    parser.add_argument('--loss-types', type=str, nargs='+', 
                       default=['nll', 'cpl_ipcw', 'cpl_ipcw_batch', 'ccl_ipcw', 'ccl_ipcw_batch', 'pcl', 'pcl_moco'],
                       choices=['nll', 'cpl', 'cpl_ipcw', 'cpl_ipcw_batch', 'ccl_ipcw', 'ccl_ipcw_batch', 'pcl', 'pcl_moco'],
                       help='Loss types to test. Default: all. Example: --loss-types pcl pcl_moco')
    parser.add_argument('--pcl-only', action='store_true',
                       help='Shortcut to run only ProtoCoxLoss variants (pcl and pcl_moco)')
    
    args = parser.parse_args()
    
    # Handle --pcl-only shortcut
    if args.pcl_only:
        args.loss_types = ['pcl', 'pcl_moco']
    ...
    # Run benchmark
    runner = PreprocessedMIMICBenchmarkRunnerPCL(
    ...
    try:
        ...
        print(f"\n{'=' * 80}")
        print("BENCHMARK PCL COMPLETED SUCCESSFULLY")
        print(f"{'=' * 80}")
